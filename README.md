...
> 「电机完全不转」的根因已修复（速度脚原先给了静态高电平，超出厂家 98% 占空比上限），
> 修复后方向逻辑确认无误，**但功率端输出电压幅值不对**：空载实测 10.7 V，预期 23.5 V。
> 未解决的问题、怀疑方向（高边自举电容在 98% 占空比下充电不足）和下一步测法，
> 都记在 [`HANDOFF.md` 第 7 节](HANDOFF.md#7-当前未解决的问题功率端电压只有-107-v2026-09-16-晚未解决)。
